            ...
            <h3>{folders.join(" > ")}</h3>
            {
                props.albumNodes.map(node => {     
                    if(typeof node === "string"){
                        return <Photo key={node} folderName={props.path} fileName={node} setLargePhotoSrc={setLargePhotoName}/>
                    }
                    const newPath = props.path+"/"+node.folderName;
                    return <Album key={node.folderName} path={newPath} album={node}/>
                })
            }
            {largePhotoName && <LargePhoto folderName={props.path} fileName={largePhotoName} />}
        </>
    )
}
